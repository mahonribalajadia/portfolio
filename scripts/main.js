...
if (isMobile) {
    // If mobile, we stop the script here so nothing runs
    console.log("Mobile detected: Rocket cursor disabled.");
} else {
    const rocket = document.getElementById('rocket-cursor');
    let mouseX = 0, mouseY = 0, rocketX = 0, rocketY = 0;
    let angle = 0, lastX = 0, lastY = 0;
    let isHovering = false;

    // Colors requested for normal mode
    const normalFireColors = ['#ff4d00', '#ff8800', '#ffcc00', '#ffffff'];
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        // Standard particles only trigger on move
        if (!isHovering && (Math.abs(e.clientX - lastX) > 2 || Math.abs(e.clientY - lastY) > 2)) {
            spawnAtTail();
        }
        lastX = e.clientX; lastY = e.clientY;
    });

    // Hover Auto-fire loop
    setInterval(() => {
        if (isHovering) {
            spawnAtTail();
        }
    ...
    function spawnAtTail() {
        ...
        const sideShift = isHovering ? 2 : 0;
    ...
    function spawn(x, y) {
        ...
        const color = isHovering 
            ? (Math.random() > 0.8 ? '#ffffff' : '#00f2ff') 
            : normalFireColors[Math.floor(Math.random() * normalFireColors.length)];

        const size = isHovering ? 5 : 4;
        ...
        const radians = (angle + 90) * (Math.PI / 180);
        const v = isHovering ? 65 : 35;
        const dx = Math.cos(radians) * v + (Math.random()-0.5)*20;
        const dy = Math.sin(radians) * v + (Math.random()-0.5)*20;
